...
def req(method, url, **kwargs):
    # Simple retry/backoff with logging
    delay = 0.5
...
# --- Change plan (create/update) ---
def compute_change_plan(merged, state):
...
def children_have_helper(page_id):
    # Check first 100 children for helper text
...
def main():
    ...
    # Show plan
    plan = compute_change_plan(merged, state)
    ...
    # Ensure helper infra: Letters parent, Diagnostics, Acceptance, Console, Rollout
    letters_parent_id = state.get("letters_parent_id")
    if not letters_parent_id:
        payload = create_page_payload("Letters", parent,
            ...
            role="owner")
        resp = req("POST","https://api.notion.com/v1/pages", data=json.dumps(payload))
        if resp.status_code < 300:
            letters_parent_id = resp.json()["id"]; state["letters_parent_id"]=letters_parent_id; print(f"Created Letters parent page.")
        else:
            print("ERROR creating Letters parent"); sys.exit(1)

    # Diagnostics DB
    diagnostics_db_id = state.get("diagnostics_db_id")
    if not diagnostics_db_id:
        diag_schema = {"Page":"title","Severity":"select","Message":"rich_text","Resolved":"select"}
        ...
        if r.status_code < 300:
            diagnostics_db_id = r.json()["id"]; state["diagnostics_db_id"]=diagnostics_db_id; print("Created Diagnostics DB.")
        else:
            print("ERROR creating Diagnostics DB"); sys.exit(1)
    ...
    if not acceptance_db_id:
        ...
        r = req("POST","https://api.notion.com/v1/databases", data=json.dumps(payload))
        if r.status_code < 300:
            acceptance_db_id = r.json()["id"]; state["acceptance_db_id"]=acceptance_db_id; print("Created Acceptance DB.")
        else:
            print("ERROR creating Acceptance DB"); sys.exit(1)

    # Builder's Console
    console_page_id = state.get("builders_console_id")
    if not console_page_id:
        payload = create_page_payload("Builder’s Console", parent,
            icon=globals_cfg.get("icons",{}).get("owner"),
            cover=globals_cfg.get("covers",{}).get("owner"),
        ...
        if rc.status_code < 300:
            console_page_id = rc.json()["id"]; state["builders_console_id"]=console_page_id; print("Created Builder’s Console.")
        else:
            print("ERROR creating Builder’s Console"); sys.exit(1)
        console_children = [
        ...
        req("PATCH", f"https://api.notion.com/v1/blocks/{console_page_id}/children", data=json.dumps({"children":console_children}))

    # Create/update pages with inline helpers where needed
    page_ids = state.get("pages", {})
    helper_targets = []  # list of (title, page_id, reason)
    for p in merged.get("pages", []):
        title = p["title"]; role = p.get("role"); helper_text = None
        if p.get("parent"):
            helper_text = f"Move this page under “{p['parent']}” manually (API cannot re-parent)."
        payload = create_page_payload(title, parent, icon=p.get("icon"), cover=p.get("cover"),
                                      ...
                                      role=role, helper_text=helper_text)
        if title in page_ids:
            page_id = page_ids[title]
            if (not args.update) and (not p.get("parent")):
                # no update requested; skip patch
                pass
            else:
                # Patch icon/cover; append children including helper
                patch = {"icon": payload.get("icon"), "cover": payload.get("cover")}
                req("PATCH", f"https://api.notion.com/v1/pages/{page_id}", data=json.dumps(patch))
                req("PATCH", f"https://api.notion.com/v1/blocks/{page_id}/children", data=json.dumps({"children": payload.get("children", [])}))
            ...
            if r.status_code >= 300:
                ...
                continue
            page_id = r.json()["id"]; page_ids[title]=page_id; print(f"[CREATE] Page: {title}")
        if p.get("parent"):
            helper_targets.append((title, page_id, f"Move under “{p['parent']}”"))
            # Also add link_to_page inside intended parent for convenience (if parent exists later)
            par_id = page_ids.get(p["parent"])
            if par_id:
                req("PATCH", f"https://api.notion.com/v1/blocks/{par_id}/children", data=json.dumps({"children":[{"object":"block","type":"link_to_page","link_to_page":{"type":"page_id","page_id":page_id}}]}))

    state["pages"] = page_ids

    # Create/update databases and DB Setup helper pages
    ...
    for db_name, spec in schemas.items():
        ...
        if db_name in db_ids:
            ...
            payload = {"parent":{"type":"page_id","page_id":parent},"title":[{"type":"text","text":{"content":db_name}}],
                       "properties":{}}
            # Build properties
            # Simplified: map title/select/multi_select/rich_text/date/url/phone_number/number
        ...
        if setup_title in db_setup_pages:
            ...
            if r.status_code >= 300:
                ...
                # Link to the database from the setup page
    ...
    # Letters Index + pages (unchanged from 3.5.2 aside from idempotency)
    letters_idx_id = state.get("letters_index_db_id")
    if not letters_idx_id:
        ...
        r = req("POST","https://api.notion.com/v1/databases", data=json.dumps(payload))
        if r.status_code < 300:
            letters_idx_id = r.json()["id"]; state["letters_index_db_id"]=letters_idx_id; print("Created Letters Index DB.")
        else:
            print("ERROR creating Letters Index DB"); sys.exit(1)
    ...
    for L in merged.get("letters", []):
        ...
        if title in letters_state and not args.update:
            page_id = letters_state[title]["page_id"]; page_url = letters_state[title]["url"]
            ...
            children = [
                ...
                {"object":"block","type":"link_to_page","link_to_page":{"type":"database_id","database_id":letters_idx_id}}
            ]
            ...
            res = r.json(); page_id=res["id"]; page_url=res.get("url","")
            letters_state[title]={"page_id":page_id,"url":page_url,"audience":L.get("Audience"),"category":L.get("Category")}
            print(f"[LETTER] Created: {title}")
        # Index row (append-only)
        ...
        req("POST","https://api.notion.com/v1/pages", data=json.dumps({"parent":{"type":"database_id","database_id":letters_idx_id},"properties":props}))
    state["letters_pages"]=letters_state

    # Populate Diagnostics from YAML
    ...
    # Rollout Summary: link to all pages that still have helper notes
    ...
    if not rollout_id:
        ...
        if rr.status_code < 300:
            rollout_id = rr.json()["id"]; state["rollout_page_id"]=rollout_id; print("Created Rollout Summary.")
        else:
            print("ERROR creating Rollout Summary"); sys.exit(1)

    # Build helper links: check pages with known helpers + DB Setup pages
    helper_links = []
    # page helpers: any page with a 'parent' hint -> we added helper text
    for p in merged.get("pages", []):
        if p.get("parent"):
            pid = state["pages"].get(p["title"])
            if pid and children_have_helper(pid):
    ...
    # Also add a single global icons reminder at the top (not a page link)
    icon_note = None
    if not ICON_BASE_URL:
        icon_note = "Host diamond icons and set ICON_BASE_URL in .env. Delete this reminder once icons display correctly."

    # Replace Rollout content
    children = []
    if icon_note:
        children.append(helper_callout(icon_note))
    if helper_links:
    ...
    req("PATCH", f"https://api.notion.com/v1/blocks/{rollout_id}/children", data=json.dumps({"children":children}))

    save_state(state)
    print("\nDone. Rollout now links to every page still containing a Setup Helper note. Remove the note on the page and re-run --update to refresh Rollout.")
