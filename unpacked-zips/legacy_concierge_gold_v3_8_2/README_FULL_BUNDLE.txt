...
v3.7.8 Auditor-Complete Patch
- Synced Blocks: true originals with SYNC_KEY::<KEY> markers; references are idempotent.
- Relations: added Pages Index DB; relation props point to the Index DB; seeds resolve to Index items.
- Breadcrumbs: unresolved relations write 'Intended Relation' and a visible marker.
- Idempotency: Navigation, Mobile Tips, and Letters/Legal content gated by markers; no duplicates on re-run.
- Hero: single-pass with HERO_MARKER.
- Grid: balanced columns for sparse children.
- Rollout Summary: live summary page updated on --update and deploy tail.
- Error Handling: imports and URL handling hardened.

v3.7.8C Script Simplification
- Single launcher: deploy/deploy.py (no version in filename)
- No --dry-run flag; the script always prints a preview and asks (y/n) before making changes
- Auditor fixes finalized: idempotent add-ons, grid balance, 4xx diagnostics
