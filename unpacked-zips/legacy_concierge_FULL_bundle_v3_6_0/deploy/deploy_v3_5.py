...
def create_page(parent_id, title, icon=None, cover=None, description=None, disclaimer=None, helper=None):
...
def main():
    ...
    for p in merged.get("pages", []):
        ...
        desc=p.get("description"); disc=p.get("disclaimer")
        ...
        pid = create_page(parent, title, icon, cover, desc, disc, helper)
    ...
    for db_name, schema in (merged.get("db") or {}).get("schemas", {}).items():
        ...
        if dbid and schema.get("seed_rows"):
            insert_db_rows(dbid, schema["seed_rows"])
    ...
    if rn:
        ...
        if not args.dry_run:
            pid = create_page(parent, "Release Notes", resolve_icon("emoji:🗒️"), None, None, None, None)
