...
def main():
    ...
    if start_pid:
        start_blocks=[
            {"object":"block","type":"callout","callout":{"icon":{"type":"emoji","emoji":"✨"},"rich_text":[{"type":"text","text":{"content":"First 3 steps"}}],"color":"gray_background"}},
            {"object":"block","type":"bulleted_list_item","bulleted_list_item":{"rich_text":[{"type":"text","text":{"content":"Open the Preparation Hub"}}]}},
            {"object":"block","type":"bulleted_list_item","bulleted_list_item":{"rich_text":[{"type":"text","text":{"content":"Review an Executor step"}}]}},
            {"object":"block","type":"bulleted_list_item","bulleted_list_item":{"rich_text":[{"type":"text","text":{"content":"Share the Family QR pack when ready"}}]}},
            {"object":"block","type":"heading_2","heading_2":{"rich_text":[{"type":"text","text":{"content":"Choose a path"}}]}}
        ]
        for hub in hubs:
            hid = hub_ids.get(hub)
            if not hid: continue
            start_blocks += card_tile(hub, f"Go to {hub.lower()}.", hid, role=("executor" if hub=="Executor Hub" else ("family" if hub=="Family Hub" else "owner")))
        req("PATCH", f"https://api.notion.com/v1/blocks/{start_pid}/children", data=json.dumps({"children":start_blocks}))
    ...
    for hub, hid in hub_ids.items():
        ...
        hub_blocks=[
            ...
            {"object":"block","type":"heading_2","heading_2":{"rich_text":[{"type":"text","text":{"content":"Sections"}}]}}
        ]
        for t in children_titles:
            pidc = state["pages"].get(t)
            if not pidc: continue
            # Determine role color by hub
            role = "owner" if hub=="Preparation Hub" else ("executor" if hub=="Executor Hub" else "family")
            hub_blocks += card_tile(t, None, pidc, role=role)
        # Progress Summary placeholder
        hub_blocks.append({"object":"block","type":"paragraph","paragraph":{"rich_text":[{"type":"text","text":{"content":"Tip: Open the Setup & Acceptance database and filter to this hub to see pending items."}}]}})
        # Back to Start
        if start_pid:
    ...
    # UNIVERSAL NAV: Back to Hub + Next Step for ALL child pages (including letters/legal)
    order_by_parent = {}
    for p in merged.get("pages", []):
        if p.get("parent"):
