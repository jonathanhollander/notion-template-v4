...
def url_join(base, filename):
    try:
        ...
        return base.rstrip("/") + "/" + _u.quote(filename.lstrip("/"))
    except Exception:
...
def insert_children(block_id, children):
    r = req("PATCH", f"https://api.notion.com/v1/blocks/{block_id}/children", data=json.dumps({"children":children}))
...
def ledger_add(state, feature_key, page_id, block_id, status="added"):
    ...
    if did:
        ...
        req("POST","https://api.notion.com/v1/pages", data=json.dumps({"parent":{"database_id":did},"properties":props}))
...
def init_state():
    ...
    return {
        ...
        "synced_blocks": {},
        "relations_report": [],
        "root_page_id": os.getenv("NOTION_PARENT_PAGEID"),
        "pages_index_db": None
    }
...
def ensure_root(state, merged):
    ...
    if not state.get("root_page_id"):
        state["root_page_id"] = os.getenv("NOTION_PARENT_PAGEID")
...
def rollback():
    ...
    for pid in reversed(TXN["created_pages"]):
        try:
            req("PATCH", f"https://api.notion.com/v1/pages/{pid}", data=json.dumps({"archived": True}))
        except Exception as e:
            print("[!] Failed to archive page", pid, e)
    for did in reversed(TXN["created_dbs"]):
        try:
            req("PATCH", f"https://api.notion.com/v1/databases/{did}", data=json.dumps({"archived": True}))
        except Exception:
            pass
...
def load_all_yaml(yaml_dir=None):
    ...
    for yaml_file in yaml_files:
        if yaml_file.name.startswith("README"):
            continue
        ...
        try:
            ...
            # Merge database schemas and seed rows
            if "db" in data:
                if "schemas" in data["db"]:
                    merged["db"]["schemas"].update(data["db"]["schemas"])
                if "seed_rows" in data["db"]:
                    for db_name, rows in data["db"]["seed_rows"].items():
                        if db_name not in merged["db"]["seed_rows"]:
                            merged["db"]["seed_rows"][db_name] = []
                        merged["db"]["seed_rows"][db_name].extend(rows)
    ...
    print(f"Merged {len(yaml_files)} YAML files")
    print(f"Total pages: {len(merged['pages'])}")
    print(f"Total databases: {len(merged['db']['schemas'])}")
    
    return merged

def parse_args():
    """Parse command line arguments"""
    parser = argparse.ArgumentParser(description='Deploy Notion Template')
    parser.add_argument('--dir', default='../split_yaml', help='YAML directory')
